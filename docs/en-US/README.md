...
- **Medication library** — Manage personal stock with three inventory forms: unopened boxes, opened blisters, and loose pills; intake is deducted automatically in "loose → blister → box" order and a new pack is opened when stock runs low; built-in drug database searchable by name/category/tags, stock adjustments, and a change log
- **Per-period dosing** — Set a separate dose for each medication in the morning/afternoon/evening/bedtime (e.g. 1 tablet in the morning, 0.5 at night); the matching dose is auto-filled when logging intake, and can be edited manually; leave blank to skip that period
- **Milligram dose conversion** — After setting the dose per tablet (e.g. 250mg/tablet), intake records, effect curves, and AI summaries automatically show the total milligram dose (e.g. `Lithium 1 tablet (250mg)`)
...
- **AI mood summary** — Connect to any OpenAI-compatible endpoint to automatically read mood, medication, sleep, and event records from a selected time range (last 1 week / 2 weeks / 1 month / custom) and generate a structured observation summary; supports streaming output, reasoning models (deepseek-r1 etc.), custom prompts, generation parameters (temperature/top_p/max_tokens/seed etc.), and saved history with regeneration
- **Medication reminders** (Android only) — A system notification (sound + vibration + light) fires at each scheduled intake time; all medications due at the same time are **combined into one notification** instead of separate pop-ups; schedules are persisted by time point, so reminders fire reliably even when the app is **fully closed, screen is locked, or the device has rebooted**; can request the system's exact-alarm permission for on-time delivery
...
- **System reminders** (Android only) — Add events to the system calendar and set up medication reminders
...
### Medication Reminders (optional, Android)

In Medications → Add/Edit → set intake times → Save to enable reminders. For the most reliable delivery:

- Allow the "notifications" permission when prompted on first launch (Android 13+);
- On Android 12 and below, follow the on-screen prompt to grant the exact-alarm permission in system "Alarms & reminders" settings;
- On some vendor systems (Xiaomi/Huawei), enable **auto-start** for the app in system settings and set battery policy to **unrestricted**, so background alarms are not killed.
...
- The AI summary is an optional feature: only when you configure an endpoint and tap "Generate" are the selected time-range records sent to the OpenAI-compatible API you specify; **the API key is stored only on your device** and never sent to any server.
